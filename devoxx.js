...
function handleFileSelect(evt) {
	...
	for (var i = 0, f; f = files[i]; i++) {
			...
			return function(onloadEvent) {
				...
				for (var j = 0, result; result = tweets.results[j]; j++) {
					...
					if (result.geo !== null) {
						...
						addPosition(result.geo.coordinates[0],result.geo.coordinates[1]);
					}
		...
		// set null as second arg if u want the tweets to be presented in the reverse order
		tweets.insertBefore(navigatorDiv, document.getElementById("tweets").firstChild);
	}
}
